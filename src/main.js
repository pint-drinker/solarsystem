...
class SolarSystem {
    ...
    updateDate() {
        ...
        this.current_time += NUMBER_OF_CALCULATIONS_PER_FRAME * DELTA_T * Math.pow(10, 3);
    }
    ...
    updateTime() {
        this.time_per_frame = NUMBER_OF_CALCULATIONS_PER_FRAME * DELTA_T;  // in seconds, going to need to scale this later
        this.time_factor = this.time_per_frame * FRAME_RATE;
        this.time_info_holder.innerHTML = 'Time Step(s): ' + DELTA_T.toFixed(4).toString() + '<br />Steps per frame: ' +
            NUMBER_OF_CALCULATIONS_PER_FRAME.toString() + '<br />Time Per Frame: ' + getTimeString(this.time_per_frame) +
    ...
    setPlanetDeltaT(body) {
        if (body.host) {
            DELTA_T = 2 * Math.PI / current_target.host.omega / NUMBER_OF_CALCULATIONS_PER_FRAME / FRAMES_TO_ROTATE;
        } else {
            if (body.name == 'mercury') {
                DELTA_T = 2 * Math.PI / current_target.omega / NUMBER_OF_CALCULATIONS_PER_FRAME / FRAMES_TO_ROTATE / 60;
            } else if (body.name == 'venus') {
                DELTA_T = 2 * Math.PI / current_target.omega / NUMBER_OF_CALCULATIONS_PER_FRAME / FRAMES_TO_ROTATE / 25;
            } else if (body.name == 'hermes') {
                DELTA_T = 1 / FRAMES_TO_ROTATE / NUMBER_OF_CALCULATIONS_PER_FRAME;
            } else {
                DELTA_T = 2 * Math.PI / current_target.omega / NUMBER_OF_CALCULATIONS_PER_FRAME / FRAMES_TO_ROTATE;
            }
        }
    }
