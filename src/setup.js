...
setupGui = function() {
	var gui = new dat.GUI();
	var parameters = 
	{time: 0, color: "#ffff00" };  // time scaling units are in weeks

	var top = gui.addFolder('Time Scaling');

  var timeGUI = 
  top.add(parameters, 'time' ).min(INITIAL_TIME).max(END_TIME).step(3600).name('Time Val').listen();
  timeGUI.onChange( function(value) {
    TIME = value;
  });
}
